...
bot = discord.Bot(activity = discord.Game(name="/fueled"),status=discord.Status.dnd)
...
async def delete_tag(ctx,index_or_name):
    if lock_status() == 'unlocked':    
        ...
        if role is not None and role.name == "staff":
            await ctx.respond("Gotcha , deleting tag...")
            data = read_file()
            tags = list(data.keys())
            ...
            await ctx.respond("Request denied , you are not the moderator.")
    else:
        await ctx.respond("Sorry , i will not be responding to any commands")
...
async def delete_all_tags(ctx,confirm):
    if ctx.author.id == 518118892317442059:
        ...
        await ctx.respond("Request denied , you are not Mg")

@bot.slash_command()
async def fueled_lock(ctx,status):
...
async def fueled(ctx):
    ...
    ```/fueled``` : Get this message, No parameter\n
